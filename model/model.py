...
class CoNTGenerator(nn.Module):
    ...
    def pad2max_len(self, input_tensor, max_len):
        ...
        pad_tensor = torch.full([input_tensor.shape[0], input_tensor.shape[1], pad_size], self.pad_id,
                                device=input_tensor.device ,dtype= torch.long) # 版本问题 .long()改成 ,dtype= torch.long
    ...
    def forward(self, src_inp, target_inp, target_outp, aug_spell, aug_gec ):
        ...
        encoder_hidden_states = encoder(src_inp, src_pad_mask)[0]
        ...
        maxlen = max(aug_spell.size(2), cand_ids.size(2), samples_from_batch.size(2)) # , aug_gec.size(2), src_cont.size(2)
        # 调整seqlen
        ...
        if samples_from_batch.size(2) < maxlen:
            ...
            samples_from_batch = samples_from_batch[:, :, :maxlen]        
        # cand_len = cand_ids.size(2)
        # samples_len = samples_from_batch.size(2)
        # if samples_len < cand_len: # 最大为candidate_len
    ...
    def changeWarmup(self, target=False):
        self.args.warmup = target

    @torch.no_grad()
    def generate_(self, input_ids, num_beams, max_length): # &
        self.generator.eval()
        result = self.generator.generate(input_ids=input_ids, 
