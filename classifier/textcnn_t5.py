# -*- coding: utf-8 -*-
# source code: https://github.com/laihuiyuan/pre-trained-formality-transfer/blob/main/classifier/textcnn.py
import os
import sys
import time
import argparse
...
def main():
    ...
    with open('./data/{}/train/formal'.format(opt.dataset),'r') as f:
        for line in f.readlines():
            train_tgt.append(tokenizer.encode(line.strip())[:opt.max_len])
    with open('./data/{}/tune/informal'.format(opt.dataset),'r') as f:
        for line in f.readlines():
            valid_src.append(tokenizer.encode(line.strip())[:opt.max_len])
    with open('./data/{}/tune/formal'.format(opt.dataset),'r') as f:
        for line in f.readlines():
    ...
    if os.path.exists(f'./checkpoints/{opt.dataset}_embedding.pt'):
        ...
        torch.save(embedding, f'./checkpoints/{opt.dataset}_embedding.pt')
        ...
        for idx, batch in enumerate(train_loader):
            ...
            if optimizer.steps % opt.eval_step == 0:
                ...
                if avg_acc < valid_acc:
                    avg_acc = valid_acc
                    save_path = 'checkpoints/t5_textcnn_{}.chkpt'.format(opt.dataset)
